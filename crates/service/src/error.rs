...
impl From<AuthError> for ServiceError {
    fn from(err: AuthError) -> Self {
        match err {
            ...
            AuthError::HashingFailed | AuthError::HashingPanic => {
    ...
    fn from(err: RepositoryError) -> Self {
        match err {
            RepositoryError::NotFound => {
                ServiceError::Internal("unexpected not found at repository layer".into())
            }
            RepositoryError::UniqueViolation { constraint } => ServiceError::Conflict {
                message: format!("a record with this value already exists ({})", constraint),
            },
