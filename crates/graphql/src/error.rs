...
pub fn to_graphql_error(err: ServiceError) -> Error {
  let (message, code) = match &err {
      ServiceError::Unauthenticated => {
        ("Authentication required".into(), "UNAUTHENTICATED")
      }
      ServiceError::InvalidCredentials => {
        ("Invalid credentials".into(), "INVALID_CREDENTIALS")
      }
      ServiceError::TokenExpired => {
        ("Token expired".into(), "TOKEN_EXPIRED")
      }
      ServiceError::InvalidToken => {
        ("Invalid token".into(), "INVALID_TOKEN")
      }
      ServiceError::Forbidden { reason }  => {
        (format!("Forbidden: {reason}"), "FORBIDDEN")
      }
      ...
      | ServiceError::OrganizationNotFound { .. } => {
        (err.to_string(), "NOT_FOUND")
      }
      ServiceError::Conflict { message } => {
        (message.clone(), "CONFLICT")
      }
