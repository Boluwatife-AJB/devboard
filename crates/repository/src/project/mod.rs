...
pub(crate) fn model_to_domain(
  ...
  Ok(Project { 
    id: devboard_domain::ProjectId::from(model.id), organization_id: devboard_domain::OrganizationId::from(
      model.organization_id
    ), 
    team_id: devboard_domain::TeamId::from(model.team_id), name: model.name, 
    key: model.key, 
...
pub(crate) fn membership_to_domain(
  ...
  Ok(ProjectMembership { 
    ...
    user_id: devboard_domain::UserId::from(model.user_id), role_override, 
    added_at: model.added_at.into() 
  })
}

pub(crate) fn str_to_project_role(
  s: &str,
) -> Result<ProjectRole, RepositoryError> {
  match s {
      "OWNER" => Ok(ProjectRole::Owner),
      "ADMIN" => Ok(ProjectRole::Admin),
      "CONTRIBUTOR" => Ok(ProjectRole::Contributor),
      "VIEWER" => Ok(ProjectRole::Viewer),
      other => Err(RepositoryError::InvalidData { message: format!("unknown project role: {other:?}") 
    }),
  }
}
