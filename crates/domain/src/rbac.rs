use crate::{project::{ProjectMembership, ProjectRole}, team::TeamMembership};
...
pub fn has_project_permission(
  ...
  resolve_project_role(team_membership, project_membership).map(|role| role.at_least(required)).unwrap_or(false)
}
...
mod tests {
  ...
  use chrono::Utc;
  use uuid::Uuid;
  use crate::{ids::{ProjectId, TeamId, UserId}, team::TeamRole};

  

  fn user_id() -> UserId { UserId(Uuid::new_v4())}
  fn team_id() -> TeamId {TeamId(Uuid::new_v4())}
  fn project_id() -> ProjectId {ProjectId(Uuid::new_v4())}
  ...
  fn has_permission_respects_minimum_role() {
    ...
    assert!(has_project_permission(Some(&tm), None, ProjectRole::Contributor));
